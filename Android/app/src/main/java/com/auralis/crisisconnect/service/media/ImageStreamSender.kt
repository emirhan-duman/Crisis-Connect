...
private const val TAG = "ImageStreamSender"
...
class ImageStreamSender(
    ...
    init {
        ...
        chunkSize = ImageStreamConstants.computeChunkSize(expectedBytes)
        ...
        tempDir = File(context.cacheDir, ImageStreamConstants.OUTBOX_DIR).apply { mkdirs() }
        persistChunks()
    ...
    private suspend fun sendInit() {
        ...
        Log.i(
            TAG,
            "Sending image:init uuid=$uuid session=$sessionCode totalBytes=${contentBytes.size} " +
                "chunkSize=$chunkSize chunkCount=$totalChunks encrypted=${aesKey != null}"
        )
        val init = JSONObject().apply {
            ...
            put("mime", mime)
            if (width != null) put("width", width) else put("width", JSONObject.NULL)
            if (height != null) put("height", height) else put("height", JSONObject.NULL)
        ...
        if (!frameWriter(init)) {
            failTransfer("Failed to send IMAGE_INIT")
            return
        }
        pumpJob?.cancel()
    ...
    private suspend fun pumpChunks() {
        while (scope.isActive && !finished) {
            while (chunkStates.count { it.inFlight && !it.acknowledged } < ImageStreamConstants.WINDOW_SIZE) {
    ...
    private suspend fun sendChunk(state: ChunkState) {
        if (state.attempts >= ImageStreamConstants.MAX_RETRY) {
            failTransfer("Chunk ${state.index} exceeded retries")
            return
        }
        val payload = JSONObject().apply {
            put("type", "image:chunk")
            put("uuid", uuid)
        ...
        if (state.index == 0) {
            Log.d(TAG, "Sent first image chunk uuid=$uuid bytes=${state.data.size}")
        }
        state.inFlight = true
        state.attempts += 1
    ...
    private suspend fun sendFinish() {
        ...
        val finish = JSONObject().apply {
            put("type", "image:finish")
            put("uuid", uuid)
        }
        if (!frameWriter(finish)) {
            failTransfer("Failed to send IMAGE_FINISH")
        }
    }
    ...
    fun handleAck(receivedUpTo: Int, missing: List<Int>) {
        Log.d(
            TAG,
            "Received image:ack uuid=$uuid receivedUpTo=$receivedUpTo missing=${missing.joinToString(",")}"
        )
        if (receivedUpTo >= 0) {
    ...
    fun handleResume(want: List<Int>) {
        Log.w(TAG, "Received image:resume uuid=$uuid want=${want.joinToString(",")}")
        want.forEach { index ->
            chunkStates.getOrNull(index)?.let { state ->
                state.inFlight = false
                state.lastSendAt = 0L
    ...
    fun handleVerify(success: Boolean) {
        Log.i(TAG, "Received image verify uuid=$uuid success=$success")
        timeoutJob.cancel()
        if (success) {
    ...
    private fun markChunkAcked(state: ChunkState) {
        ...
        state.inFlight = false
        updateProgress(
            if (chunkStates.all { it.acknowledged }) ImageTransferState.Verifying
            else ImageTransferState.Transferring
        )
    }
    ...
    private suspend fun monitorTimeouts() {
        while (scope.isActive && !finished) {
            ...
            chunkStates.filter { it.inFlight && !it.acknowledged }
                .filter { now - it.lastSendAt >= ImageStreamConstants.CHUNK_TIMEOUT_MS }
                .forEach { state ->
                    state.inFlight = false
